...
important <- sanfrancrime %>%
  ...
  select(Category, DayOfWeek, Date, PdDistrict, Resolution, hour, month)
...
Because this dataset is directly from the San Francisco Police Department, Crime is not recorded by Neighborhood but Police District. To get a better understanding of how this may affect crime, we decided to visualize these districts as well as take a loot at the number of crimes reported per district. Because there is not a clear pattern between area and crime count, we will be using proportions when analyzing crime rates from district to district. 
```{r map-of-SF}
library(sf)
...
test_sanfrancrime <- test_sanfrancrime%>%
  mutate(weektype = factor(weektype, levels = c("Weekend","Weekday")))

test_sanfrancrime <- test_sanfrancrime%>%
  mutate(timerange = factor(timerange, levels = c("Nighttime","Daylight")))
test_sanfrancrime%>%
  select(weektype, timerange)
...
$H_0:$ Property crime is equally likely to occur during autumn compared to other
seasons

$H_a:$ Property crime is more likely to occur during autumn than other seasons
...
t.test(data = summercheck,
       is_Property ~ is_autumn,
       conf.level = 0.95,
       alternative = "two.sided",
       ver.equal = FALSE)
```
Given the p-value of 0.08554 is greater than the alpha level of 0.05, we fail
...
$H_0$: Predicted target has the SAME likelihood of violent crime occuring than
the sample population.

$H_a$: Predicted target has a GREATER likelihood of violent crime occuring than
the sample population.

$\alpha$ of 0.05    


```{r another attempted t-test}
pls <- important %>% 
...
##im not sure if i did this t-test right? should it be less or greater or two-sided
```
...
The first t- test addressed the null hypothesis that crime is committed equally
during daylight and nighttime hours on the week and weekend.
