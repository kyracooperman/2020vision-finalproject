...
```{r mihir creating time and filtering for important variables, echo = FALSE}
...
```{r categorizing crime by type, echo = FALSE}
important <- important %>% 
...
```{r random visualizations, echo = FALSE}
...
```{r map-needs-geometry, echo = FALSE}
library(sf)
...
#need to change area bc doesn't make sense rn lol
ggplot(sfran) +
  geom_sf(aes(fill = shape_area)) +
  scale_fill_gradient(low = "#fff7f3", high = "#49006a") +
  labs(title = "Certain SF counties have more crime",
       fill = "Total number of crimes") + 
    theme_bw()

```

links to use for the map:
...
```{r PD vs Violent Crime, echo = FALSE}
...
```{r day of week and number of crimes, echo = FALSE}
library(forcats)
sanfrancrime <- sanfrancrime%>%
  mutate(DayOfWeek = factor(DayOfWeek, labels = c("Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"),levels = c(0,1,2,3,4,5,6)))
...
```{r day and crimetype, echo = FALSE}
...
categorical variables to determine if there is the relationship between them is 
