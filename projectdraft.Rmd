...
```{r}
library(broom)
mod_aug<- augment(mod2)

##independence
ggplot(data = mod_aug, 
       aes(x = 1:nrow(sanfrancrime), 
           y = .resid)) + 
  geom_point() + 
  labs(x = "Index", y = "Residual", title = "Residuals Are Independent")
...
ggplot(mod_aug, mapping = aes(x = .fitted, y = .resid)) +
  ...
  labs(x = "Predicted Height", y = "Residuals", 
       title = "Residuals Are Linear With Constant Variance ")

##normality
ggplot(mod_aug, mapping = aes(x = .resid)) +
  geom_histogram(bins = 15) + 
  labs(x = "Residuals", title = "Residuals are Normally Distributed")
```
