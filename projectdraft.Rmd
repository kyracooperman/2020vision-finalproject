...
If we were to continue work on the project, we would add to our analysis by introducing data from different cities that are comparable to SF. Common major cities that post-college students flock to include NYC and DC, among others. It would be interesting to see the parallels in crime rates, as for many college students, traveling to their first job post-grad will be their first taste of independence and financial freedom -- thus, safety is an important factor to take into consideration. We would also introduce the most populous cities from all over the world, like Shanghai or São Paulo.
...
should look like a paper from poli sci

major links to use for the map:
