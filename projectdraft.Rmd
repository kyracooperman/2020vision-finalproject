...
Because this dataset is directly from the San Francisco Police Department, Crime is not recorded by Neighborhood but Police District. To get a better understanding of how this may affect crime, we decided to visualize these districts as well as take a look at the number of crimes reported per district [5-7]. Because there is not a clear pattern between area and crime count, we will be using proportions when analyzing crime rates from district to district. 
...
This visualization shows the coordinate location of every crime committed. There is a high concentration of crime in the Northeast region of San Francisco. Comparing this to our first map can further tell us that Tenderloin, Central, Northern, and Mission are the districts where this concentration is. Property seems to be the most common crime followed by Miscellaneous and then Violent Crime. However, this cannot be proven just from this visualization, so we need to visualize more factors. 
...
ggplot(data = important, mapping = aes(x = timerange)) +
  ...
  labs(x = "Time of the Day", y = "Count", 
       ...
       subtitle = "TThe Most Violent Also Crime Occurs in the Latter Half of the Day",
       fill= "Crime Type")
```
...
Followed by property and miscellaneous crime, violent crime was the most prevalent type of crime committed. Because violent crime typically indicates a worse degree of crime, we believed this would be an interesting relatonship to analyze. By looking at the table and bar plot, it is clear that Ingleside, Bayview, and Tenderloin have the highest rates of violent crime. However, Mission, Southern, and Bayview have the highest number of violent crimes. Park and Richmond both have the lowest rates and total numbers of violent crimes. For all police districts, the percentage of violent crimes is lower than 18%. 
...
ggplot(data = important, mapping = aes(x = season)) +
  ...
  labs(title = "Distribution of Crime Suggests More 
       ...
       y = "Count")
```
We took a deep investigation of crime type vs day and time. This can provide insight to what individuals can possibly do to be safer. However, we also wanted to visualize this on a broader perspective and see if certain seasons had a greater amount of a certain type of crime or overall crime. After analyzing this bar plot, we can conclude that the ditribution of crime type is pretty similar across all seasons. Because this visualization is the number of crimes throughout the whole year, we are interested if the higher number of property crime in autumn and winter is significant. 

# Chi-Square Test

The visualization between crime type and day left us unsure if a relationship existed between these two variables. So, we will be performing a Chi-Squared test between the crime types and categorical time of day to determine if there is the relationship between them is statistically significant.
...
Before using logistic regression, we checked that the necessary conditions of
linearity, independence, normality, and equal variance were met. 
...
The logistic regression predicted that the combination of Saturday and Dawn would have the greatest likelihood of a violent crime occurring. As a result, we are exploring the significance of this relationship with a t-test.

$H_0$: Saturdays at Dawn have the SAME likelihood of a violent crime occurring as the sample population.

$H_a$: Saturdays at Dawn have a GREATER likelihood of a violent crime occurring than the sample population.
...
The logistic regression predicted that the combination of Sunday and Night would have the greatest likelihood of a property crime occurring. As a result, we are exploring the significance of this relationship with a t-test.

$H_0$: Sunday at Night have the SAME likelihood of a property crime occurring as the sample population.

$H_a$: Saturdays at Night have a GREATER likelihood of a property crime occurring than the sample population.
...
$H_0:$ Property crime is equally likely to occur during colder seasons Autumn and Winter than compared to other
seasons

$H_a:$ Property crime is more likely to occur during the colder seasons (Autumn and Winter)
...
The reference level (not sure if that’s the correct term) is Sunday at Dawn. While holding the day of the week constant, the log odds of a violent crime occuring decreases by 0.471 if morning, 0.608 if afternoon, and 0.381 if it is night. While holding the time range of the day constant, the log odds of a violent crime occuring decreases by 0.071 if Monday, 0.021 if Tuesday, 0.114 if Wednesday, 0.145 if Thursday, and 0.148 if it is Friday. However, the log-odds increase by 0.079 if it is Saturday. According to the model, the log-odds of a violent crime occuring is greatest when it is Saturday at Dawn and the least when it is Friday at Afternoon.
...
The reference level (not sure if that’s the correct term) is Sunday at Afternoon. While holding the day of the week constant, the log odds of a violent crime occuring decreases by 0.006 it is morning. However, the log-odds increase by 0.109 if afternoon and 0.505 if it is night. While holding the time range of the day constant, the log odds of a violent crime occuring decreases by -0.058 if Monday, -0.107 if Tuesday, -0.140 if Wednesday, -0.287 if Thursday, -0.093 if Friday, and -0.048 if it is Saturday. According to the model, the log-odds of a violent crime occuring is greatest when it is Sunday at Night and the least when it is Thursday at Morning.


### Hypothesis Tests

### Test Number 1's Analysis
After creating our visualizations, we noticed that Saturday at dawn seemed to be a point of significance; thus, we wanted to see if the predicted target (Saturday at dawn) truly had a greater likelihood of violent crime occurring than the sample population.

t = -5.0675, degrees of freedom = 381.07.

Our p-value < 3.149e-07, so we reject the null hypothesis at the alpha = 0.05 significance level. Thus there is sufficient evidence to suggest that predicted target (Saturday at dawn) has a GREATER likelihood of violent crime occurring than the sample population, which confirmed our suspicions. On a policy standpoint, this means that it might be a good idea to add more police on site during dawn hours on Saturdays in particular to be able to effectively resolve and prevent crime. 

### Test Number 2's Analysis
After creating our visualizations, we noticed that Sunday nights seemed to be a time of significance. We wanted to further explore and see if this predicted target truly had a greater likelihood of violent crime occurring than the sample population.

t = -2.6675, degrees of freedom = 622.8. Our p-value is 0.00392, so we reject the null hypothesis at the alpha = 0.05 significance level. Thus there is sufficient evidence to suggest that our predicted target of Sunday nights truly had a greater likelihood of violent crime occurring than the sample population, which confirmed our suspicions. Intuitively, this makes sense, as nighttime hours lends itself to additional obscurity. On a policy standpoint, it might be a good idea to add more police on Sunday nights to be able to effectively resolve and prevent crime. 

### Test Number 3's Analysis
In our visualization relating to seasonal differences and SF crime, we noticed that winter seasons seemed to have more crime. Thus, we wanted to further explore and see if property crime is more likely to occur in the colder seasons (Autumn and Winter).

t = -2.4695, degrees of freedom = 14996. Our p-value is 0.006771, so we reject the null hypothesis at the alpha = 0.05 significance level. Thus there is sufficient evidence to suggest that property crime is more likely to occur in the colder seasons (Autumn and Winter), which confirmed our suspicions. 
...
The bar graph that shows crime rates and violent crime proportions that is faceted by police districts shows valuable insight as to which police districts are faced with the highest crime rates. The police districts of Tenderloin, Bayview, and Ingleside have the highest percentages of violent crime, while Park and Richmond were both consistent in having the lowest proportion of violent crimes. Noting the success of these districts in maintaining low levels of crimes, it could be beneficial to restructure other districts to mirror their practices.
...
- t test results and analysis TINA done 
- justifications methodology  LEAH done
